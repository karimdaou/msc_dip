...
//#############################################################################//
    ...
    for (j = 0; j < NUM - 1; j++) {
        for (k = M; k < NUM2 - 1; k++) {
            ...
            //КИНЕТИЧЕСКИЕ СЛАГАЕМЫЕ (производные по x = 0, т.к. разница psi = 0)
////////////////////////////////////////////////////////////////
// множитель 2 стоит, т.к. приращение через границу = 2 растояниям от границы до первого узла
            // += (i+1, j+1, k) // -= (i+1, j, k)
            d_v6 = (psi[i + 1][j + 1][k] - psi[i + 1][j][k]) * 2 * x[i + 1] * dz[k] / dy[j];
            grad_E[i + 1][j + 1][k] -= d_v6;
            grad_E[i + 1][j][k] += d_v6;

            // += (i+1, j+1, k+1) // -= (i+1, j, k+1)
            d_v8 = (psi[i + 1][j + 1][k + 1] - psi[i + 1][j][k + 1]) * 2 * x[i + 1] * dz[k] / dy[j];
            grad_E[i + 1][j + 1][k + 1] -= d_v8;
            grad_E[i + 1][j][k + 1] += d_v8;
/////////////////////////////////////////////////////////////////
            // += (i+1, j, k+1) // -= (i+1, j, k)
            d_v10 = (psi[i + 1][j][k + 1] - psi[i + 1][j][k]) * 2 * x[i + 1] * dy[j] / dz[k];
            grad_E[i + 1][j][k + 1] -= d_v10;
            grad_E[i + 1][j][k] += d_v10;

            // += (i+1, j+1, k+1) // -= (i+1, j+1, k)
            d_v12 = (psi[i + 1][j + 1][k + 1] - psi[i + 1][j + 1][k]) * 2 * x[i + 1] * dy[j] / dz[k];
            grad_E[i + 1][j + 1][k + 1] -= d_v12;
            grad_E[i + 1][j + 1][k] += d_v12;

            //ПОТЕНЦИАЛЬНЫЕ СЛАГАЕМЫЕ
            // нас будут интересовать только слагаемые с индексами [i + 1], т.к i = -1,
            // слагаемые с [i] - выкидываем
            // остальные индексы такие же как прежде

            // += (i+1, j, k)
            d_v14 = V[i + 1][j][k] * psi[i + 1][j][k] * 2 * x[i + 1] * dy[j] * dz[k];
            grad_E[i + 1][j][k] -= d_v14;

            // += (i+1, j+1, k)
            d_v17 = V[i + 1][j + 1][k] * psi[i + 1][j + 1][k] * 2 * x[i + 1] * dy[j] * dz[k];
            grad_E[i + 1][j + 1][k] -= d_v17;

            // += (i+1, j, k+1)
            d_v18 = V[i + 1][j][k + 1] * psi[i + 1][j][k + 1] * 2 * x[i + 1] * dy[j] * dz[k];
            grad_E[i + 1][j][k + 1] -= d_v18;

            // += (i+1, j+1, k+1)
            d_v20 = V[i + 1][j + 1][k + 1] * psi[i + 1][j + 1][k + 1] * 2 * x[i + 1] * dy[j] * dz[k];
            grad_E[i + 1][j + 1][k + 1] -= d_v20;
////////////////////////////////////////////////////////////////
            //слагаемые, входящие в производную знаменателя функционала энергии

            // += (i+1, j, k)
            d_w2 = psi[i + 1][j][k] * 2 * x[i + 1] * dy[j] * dz[k] * numerator / denumerator;
            grad_E[i + 1][j][k] += d_w2;

            // += (i+1, j+1, k)
            d_w5 = psi[i + 1][j + 1][k] * 2 * x[i + 1] * dy[j] * dz[k] * numerator / denumerator;
            grad_E[i + 1][j + 1][k] += d_w5;

            // += (i+1, j, k+1)
            d_w6 = psi[i + 1][j][k + 1] * 2 * x[i + 1] * dy[j] * dz[k] * numerator / denumerator;
            grad_E[i + 1][j][k + 1] += d_w6;

            // += (i+1, j+1, k+1)
            d_w8 = psi[i + 1][j + 1][k + 1] * 2 * x[i + 1] * dy[j] * dz[k] * numerator / denumerator;
            grad_E[i + 1][j + 1][k + 1] += d_w8;
...
//##############################################################################//
    ...
    for (i = 0; i < NUM - 1; i++) {
        for (k = M; k < NUM2 - 1; k++) {
            ...
            //КИНЕТИЧЕСКИЕ СЛАГАЕМЫЕ (производные по y = 0, т.к. разница psi = 0)
// множитель 2 стоит, т.к. приращение через границу = 2 растояниям от границы до первого узла

            // += (i+1, j+1, k) // -= (i, j+1, k)
            d_v2 = (psi[i + 1][j + 1][k] - psi[i][j + 1][k]) * 2 * y[j + 1] * dz[k] / dx[i];
            grad_E[i + 1][j + 1][k] -= d_v2;
            grad_E[i][j + 1][k] += d_v2;

            // += (i+1, j+1, k+1) // -= (i, j+1, k+1)
            d_v4 = (psi[i + 1][j + 1][k + 1] - psi[i][j + 1][k + 1]) * 2 * y[j + 1] * dz[k] / dx[i];
            grad_E[i + 1][j + 1][k + 1] -= d_v4;
            grad_E[i][j + 1][k + 1] += d_v4;

/////////////////////////////////////////////////////////////////
            // += (i, j+1, k+1) // -= (i, j+1, k)
            d_v11 = (psi[i][j + 1][k + 1] - psi[i][j + 1][k]) * dx[i] * 2 * y[j + 1] / dz[k];
            grad_E[i][j + 1][k + 1] -= d_v11;
            grad_E[i][j + 1][k] += d_v11;

            // += (i+1, j+1, k+1) // -= (i+1, j+1, k)
            d_v12 = (psi[i + 1][j + 1][k + 1] - psi[i + 1][j + 1][k]) * dx[i] * 2 * y[j + 1] / dz[k];
            grad_E[i + 1][j + 1][k + 1] -= d_v12;
            grad_E[i + 1][j + 1][k] += d_v12;
////////////////////////////////////////////////////////////////
            //ПОТЕНЦИАЛЬНЫЕ СЛАГАЕМЫЕ
            // нас будут интересовать только слагаемые с индексами [j + 1], т.к j = -1,
            // слагаемые с [j] - выкидываем
            // остальные индексы такие же как прежде

            // += (i, j+1, k)
            d_v15 = V[i][j + 1][k] * psi[i][j + 1][k] * dx[i] * 2 * y[j + 1] * dz[k];
            grad_E[i][j + 1][k] -= d_v15;

            // += (i+1, j+1, k)
            d_v17 = V[i + 1][j + 1][k] * psi[i + 1][j + 1][k] * dx[i] * 2 * y[j + 1] * dz[k];
            grad_E[i + 1][j + 1][k] -= d_v17;

            // += (i, j+1, k+1)
            d_v19 = V[i][j + 1][k + 1] * psi[i][j + 1][k + 1] * dx[i] * 2 * y[j + 1] * dz[k];
            grad_E[i][j + 1][k + 1] -= d_v19;

            // += (i+1, j+1, k+1)
            d_v20 = V[i + 1][j + 1][k + 1] * psi[i + 1][j + 1][k + 1] * dx[i] * 2 * y[j + 1] * dz[k];
            grad_E[i + 1][j + 1][k + 1] -= d_v20;
////////////////////////////////////////////////////////////////
            //слагаемые, входящие в производную знаменателя функционала энергии
            // += (i, j+1, k)
            d_w3 = psi[i][j + 1][k] * dx[i] * 2 * y[j + 1] * dz[k] * numerator / denumerator;
            grad_E[i][j + 1][k] += d_w3;

            // += (i+1, j+1, k)
            d_w5 = psi[i + 1][j + 1][k] * dx[i] * 2 * y[j + 1] * dz[k] * numerator / denumerator;
            grad_E[i + 1][j + 1][k] += d_w5;

            // += (i, j+1, k+1)
            d_w7 = psi[i][j + 1][k + 1] * dx[i] * 2 * y[j + 1] * dz[k] * numerator / denumerator;
            grad_E[i][j + 1][k + 1] += d_w7;

            // += (i+1, j+1, k+1)
            d_w8 = psi[i + 1][j + 1][k + 1] * dx[i] * 2 * y[j + 1] * dz[k] * numerator / denumerator;
            grad_E[i + 1][j + 1][k + 1] += d_w8;
        }
    }
    ...
    for (k = M; k < NUM2 - 1; k++) {
        ...
        //КИНЕТИЧЕСКИЕ СЛАГАЕМЫЕ (производные по x и y = 0, т.к. разница psi = 0)
        // множитель 2 стоит, т.к. приращение через границу = 2 растояниям от границы до первого узла
/////////////////////////////////////////////////////////////////

        // += (i+1, j+1, k+1) // -= (i+1, j+1, k)
        d_v12 = (psi[i + 1][j + 1][k + 1] - psi[i + 1][j + 1][k]) * 2 * x[i + 1] * 2 * y[j + 1] / dz[k];
        grad_E[i + 1][j + 1][k + 1] -= d_v12;
        grad_E[i + 1][j + 1][k] += d_v12;
////////////////////////////////////////////////////////////////
        //ПОТЕНЦИАЛЬНЫЕ СЛАГАЕМЫЕ
        // нас будут интересовать только слагаемые с индексами [i + 1][j + 1], т.к i = j = -1,
        // слагаемые с [j] или [i] - выкидываем

        // += (i+1, j+1, k)
        d_v17 = V[i + 1][j + 1][k] * psi[i + 1][j + 1][k] * 2 * x[i + 1] * 2 * y[j + 1] * dz[k];
        grad_E[i + 1][j + 1][k] -= d_v17;

        // += (i+1, j+1, k+1)
        d_v20 = V[i + 1][j + 1][k + 1] * psi[i + 1][j + 1][k + 1] * 2 * x[i + 1] * 2 * y[j + 1] * dz[k];
        grad_E[i + 1][j + 1][k + 1] -= d_v20;
////////////////////////////////////////////////////////////////
        //слагаемые, входящие в производную знаменателя функционала энергии

        // += (i+1, j+1, k)
        d_w5 = psi[i + 1][j + 1][k] * 2 * x[i + 1] * 2 * y[j + 1] * dz[k] * numerator / denumerator;
        grad_E[i + 1][j + 1][k] += d_w5;

        // += (i+1, j+1, k+1)
        d_w8 = psi[i + 1][j + 1][k + 1] * 2 * x[i + 1] * 2 * y[j + 1] * dz[k] * numerator / denumerator;
        grad_E[i + 1][j + 1][k + 1] += d_w8;
    }
